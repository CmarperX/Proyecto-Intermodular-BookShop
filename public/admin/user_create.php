<?php
require_once __DIR__ . '/../../includes/config.php';
require_once __DIR__ . '/../../models/User.php';

// Only admin 
if (!Auth::isLoggedIn() || !Auth::hasRole('admin')) {
    header('Location: ../index.php');
    exit;
}

// Model instantiation
$userModel = new User($pdo);

$error = null;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {

    $data = [
        'dni' => strtoupper($_POST['dni']),
        'nombre' => $_POST['nombre'],
        'apellido' => $_POST['apellido'],
        'direccion' => $_POST['direccion'],
        'email' => $_POST['email'],
        'telefono' => $_POST['telefono'],
        'clave' => $_POST['clave'],
        'rol' => 'cliente'
    ];

    if (
        !Validation::dni($data['dni']) ||
        !Validation::text($data['nombre']) ||
        !Validation::text($data['apellido']) ||
        !Validation::text($data['direccion']) ||
        !Validation::email($data['email']) ||
        !Validation::telefono($data['telefono']) ||
        !Validation::password($data['clave'])
    ) {
        $error = "Uno de los datos introducidos no es válido";

    // check if that dni already exists
    } elseif ($userModel->dniExists($data['dni'])) {
        $error = "El DNI ya está registrado";

    // check if that email already exists
    } elseif ($userModel->emailExists($data['email'])) {
        $error = "El email ya está registrado";

    } else {

        if ($userModel->create($data)) {
            header('Location: users.php');
            exit;

        } else {
            $error = "No se pudo crear el usuario";
        }
    }
}
?>
<!-- Head -->
<?php include __DIR__ . '/../../includes/head.php'; ?>
<body>
    <!-- Navbar -->
    <?php include __DIR__ . '/../../includes/navbar.php'; ?>

    <div class="container mt-5 mb-5">
        <h2 class="mb-5">Crear usuario</h2>

        <?php if ($error): ?>
            <div class="alert alert-danger"><?= htmlspecialchars($error) ?></div>
        <?php endif; ?>

        <form method="post" autocomplete="off">
            <label for="dni">DNI:</label>
            <input class="form-control mb-2" name="dni" pattern="[0-9]{8}[A-Za-z]" title="Debe tener 8 números y una letra al final" placeholder="DNI" required>

            <label for="nombre">Nombre:</label>
            <input class="form-control mb-2" name="nombre" placeholder="Nombre" required>

            <label for="apellido">Apellido:</label>
            <input class="form-control mb-2" name="apellido" placeholder="Apellido" required>
            
            <label for="direccion">Dirección:</label>
            <input class="form-control mb-2" name="direccion" placeholder="Dirección" required>
            
            <label for="email">Email:</label>
            <input class="form-control mb-2" name="email" type="email" placeholder="Email" autocomplete="none" required>
            
            <label for="telefono">Teléfono:</label>
            <input class="form-control mb-2" name="telefono" pattern="[0-9]{9}" title="Debe tener 9 números" placeholder="Teléfono" required>
            
            <label for="clave">Contraseña:</label>
            <div class="input-group">
                <input class="form-control " name="clave" type="password" minlength="8" placeholder="contraseña" id="passwordInput" autocomplete="new-password" required>
                <button class="btn btn-outline-secondary" type="button" id="togglePassword">
                    <svg xmlns="http://www.w3.org/2000/svg" id="eyeIcon" width="28" height="28" fill="currentColor" class="bi bi-eye" viewBox="0 0 16 16">
                        <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z"/>
                        <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0"/>
                    </svg>    
                </button>
            </div>

            <div class="mt-5">
                <button class="btn btn-primary">Crear usuario</button>
                <a href="users.php" class="btn btn-primary">Cancelar</a>
            </div>
        </form>
    </div>
    <!-- Footer -->
    <?php include __DIR__ . '/../../includes/footer.php'; ?>
    <script src="../assets/js/eyePassword.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js" integrity="sha384-FKyoEForCGlyvwx9Hj09JcYn3nv7wiPVlz7YYwJrWVcXK/BmnVDxM+D2scQbITxI" crossorigin="anonymous"></script>
</body>
</html>